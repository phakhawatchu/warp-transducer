...
if not os.path.exists(os.path.join(warp_rnnt_path, "libwarprnnt" + lib_ext)):
    print(
        (
            "Could not find libwarprnnt.so in {}.\n"
            "Build warp-rnnt and set WARP_RNNT_PATH to the location of"
            " libwarprnnt.so (default is '../build')"
        ).format(warp_rnnt_path),
        file=sys.stderr,
    )
    sys.exit(1)
...
if enable_gpu:
    ...
    if (
        not os.path.exists(cuda_inc_path)
        or os.readlink(cuda_inc_path) != os.environ["CUDA_HOME"]
    ):
    ...
    if not os.path.exists(loc):
        print(
            (
                ...
                "Check your environment variables and paths?"
            ).format(loc),
            file=sys.stderr,
        )
        sys.exit(1)
